...
public class EssayService {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void updateEssay(Long userId, Long classifyId, Essay essay) {
        if (essay.getId() == null) {
            throw new GlobalException(RestCode.PARAM_ERROR.code, "文章ID不可为空");
        }
        Essay savedEssay = essayRepository.findById(essay.getId())
                .orElseThrow(() -> new GlobalException(RestCode.NOT_FOUND.code, "文章不存在"));
        if (!savedEssay.getUser().getId().equals(userId)) {
    ...
    public void altEssayState(Essay essay) {
        ...
        if (EssayStatusEnum.FORBIDDEN == essay.getState()) {
            ...
            //邮件通知作者
            mailUtil.sendSimpleMail(essay1.getUser().getEmail(), "文章审核不通过",
                    essay1.getTitle() + "审核失败，理由： " + essay.getRemark());
        } else {
            Book book = new Book();
     ...
     * @return Page<Essay>
     */
    public Page<Essay> getUserFavesEssay(Long userId, FaveTypeEnum faveType, Pageable pageable) {
